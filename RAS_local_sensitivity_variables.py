...
resultC0 = [[] for _ in range(number_of_variables)]     # baseline case 1000 ACE2 receptor
...
#############################################################
# Homeostasis cases
for Cases in range(number_of_variables):

    A = [1.0 for i in range(number_of_variables)]
    ...
    if (pateint_type == 'hypertensive'):
        x1 = 6e5 * A[1]        # Angiotensinogen  fmol/mL
        x2 = 110.0 *A[2]     # ANGI  fmol/mL
        x3 = 156.0 * A[3]     # ANGII  fmol/mL
        x4 = 92.0 * A[4]      # ANG(1-7)  fmol/mL
        x5 = 2.1e4 * A[5]     # ACE2-ANGII  fmol/mL
        x6 = 1.0 * A[6]       # ANGIV  fmol/mL
        x7 = 85.0 * A[7]      # ANGII-AT1R  fmol/mL
        x8 = 27.0 * A[8]      # ANGII-AT2R  fmol/mL

    # normotensive
    if (pateint_type == 'normotensive'):
        x1 = 6e5 * A[1]       # Angiotensinogen  fmol/mL
        x2 = 70 * A[2]        # ANGI  fmol/mL
        x3 = 28 *A[3]        # ANGII  fmol/mL
        x4 = 36 *A[4]        # ANG(1-7)  fmol/mL
        x5 = 2.1e4 *A[5]     # ACE2-ANGII  fmol/mL
        x6 = 1 *A[6]         # ANGIV  fmol/mL
        x7 = 15 *A[7]        # ANGII-AT1R  fmol/mL
        x8 = 5 *A[8]         # ANGII-AT2R  fmol/mL
    ...
    x0 = solution[0] * A[0]               # Renin  fmol/mL
    ...
    x9 = solution[9] *A[9]               # MAS-ANG(1-7)  fmol/mL
...
for variable_multiplier in range(len(multiplier_of_variable_change)):
    resultC1 = [[] for _ in range(number_of_variables)]  # variable cases 1000 ACE2 receptor
    for Cases in range(number_of_variables):
    ...
    plt.legend(['Renin', 'AGT', 'ANGI', 'ANGII', 'ANG1-7', 'ACE2-ANGII', 'ANGIV', 'ANGII-AT1R', 'ANGII-AT2R', 'MAS-ANG(1-7)'], ncol=2, loc='upper left', prop={"size":15})
    plt.xlabel('Time (day)')
    plt.ylabel('Local sensitivity of Collagen')
    plt.savefig("Collagen_"+str(multiplier_of_variable_change[variable_multiplier])+".png", dpi = 300, bbox_inches='tight')
    plt.show()
    ...
    # plt.ylim([0,0.25])
    plt.title('Multiplier = %0.2f' %multiplier_of_variable_change[variable_multiplier])
    plt.legend(['Renin', 'AGT', 'ANGI', 'ANGII', 'ANG1-7', 'ACE2-ANGII', 'ANGIV', 'ANGII-AT1R', 'ANGII-AT2R', 'MAS-ANG(1-7)'],ncol=2, loc='upper right', prop={"size": 15})
    plt.xlabel('Time (day)')
    plt.ylabel('Local sensitivity of ANGII')
    plt.savefig("ANGII_" + str(multiplier_of_variable_change[variable_multiplier]) + ".png", dpi=300,bbox_inches='tight')
    plt.show()